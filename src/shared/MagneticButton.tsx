...
const variantClasses: Record<NonNullable<MagneticButtonProps["variant"]>, string> = {
  ...
  ghost: "btn btn--secondary bg-transparent border-transparent hover:border-transparent",
};

/**
 * A CTA button that gently pulls toward the cursor when hovered
 * (magnetic effect), with a spring-based release on mouse leave.
 * Renders as a <Link>, <a>, or <button> depending on props provided —
 * except when disabled, which always forces real <button disabled>
 * semantics regardless of to/href (a disabled Link/anchor can still be
 * reached and activated via keyboard in some browsers, a disabled button
 * genuinely can't).
 */
export function MagneticButton({
  ...
  if (to) {
    return (
      <Link href={to} className={"inline-block" + className ? ` ${className}` : ''}>
        {content}
      </Link>
    );
  }
  ...
  if (href) {
    return (
      <a href={href} className={"inline-block" + className ? ` ${className}` : ''}>
        {content}
      </a>
    );
  }
