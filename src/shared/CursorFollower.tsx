...
export function CursorFollower() {
  ...
  const [variant, setVariant] = useState<"default" | "magnetic" | "text" | "link">(
    "default",
  );
  ...
  const cursorX = useSpring(ringX.get(), {
    stiffness: 300,
    damping: 30,
    mass: 0.5,
  });
  const cursorY = useSpring(ringY.get(), {
    stiffness: 300,
    damping: 30,
    mass: 0.5,
  });
  const cursorWidth = useSpring(32, { stiffness: 300, damping: 30, mass: 0.5 });
  const cursorHeight = useSpring(32, {
    stiffness: 300,
    damping: 30,
    mass: 0.5,
  });
  const cursorMarginLeft = useSpring(-16, {
    stiffness: 300,
    damping: 30,
    mass: 0.5,
  });
  const cursorMarginTop = useSpring(-16, {
    stiffness: 300,
    damping: 30,
    mass: 0.5,
  });
  ...
  useEffect(() => {
    ...
    const handleMove = (e: MouseEvent) => {
      ...
      // Use ref to check visibility to avoid cascading renders
  ...
  // Update dedicated cursor motion values based on current state
  useEffect(() => {
  ...
  // Also update continuously when not magnetized to follow the mouse
  useEffect(() => {
    if (!isMagnetic) {
      const unsubscribeX = ringX.onChange((v) => cursorX.set(v));
      const unsubscribeY = ringY.onChange((v) => cursorY.set(v));
      return () => {
        unsubscribeX();
        unsubscribeY();
      };
  ...
  return (
    <div
      ...
      aria-hidden="true"
    >
      {/* Single animated cursor container that morphs between states */}
      <motion.div
        className="absolute"
        style={{
          x: cursorX,
      ...
      >
        {/* Corner borders for default and magnetic states */}
        {!isText && !isLink && (
          <>
            <div
              className="absolute top-0 left-0 border-t border-l border-white/80 rounded-tl-md"
              style={{
                width: isMagnetic ? 20 : 10,
                height: isMagnetic ? 20 : 10,
              }}
            />
            <div
        ...
        {/* Full border for text state */}
        {isText && (
          <div className="absolute inset-0 rounded-md border border-white/80" />
        )}
      </motion.div>
      ...
      {/* Link underline line - stays in place, grows from entry point */}
      {isLink && linkAnimation && (
        ...
        >
          {/* Left segment: from entryX to rect.left */}
          <motion.div
            className="absolute border-b-1 border-white/80"
            style={{
              left: 0,
              top: 0,
          ...
          />
          {/* Right segment: from entryX to rect.right */}
          <motion.div
            className="absolute border-b-1 border-white/80"
            style={{
