...
interface SpinViewerProps {
  ...
  frames?: string[];
  /**
   * URL (relative or absolute) of a video to extract frames from client-side.
   * When supplied, takes precedence over any `frames` prop.
   */
  videoSrc?: string;
  /** Number of frames to extract when using `videoSrc`. Defaults to 24. */
  videoFrameCount?: number;
  /** Accessible description of what's shown, e.g. "18k gold bangle worn on wrist" */
  alt: string;
  className?: string;
}
...
/**
 * Drag-to-rotate frame-sequence viewer, built for the on-model 360° shots
 * described in the PRD (real footage cropped to the relevant body part per
 * category — hand, ear, neck-to-chest — rather than a full 3D model).
 * Not a 3D renderer: it just scrubs through a preloaded image sequence.
 * Frames come from /admin/frame-extractor (lib/video-to-frames.ts), which
 * turns a raw product video into the array this component expects.
 */
export function SpinViewer({ frames: framesProp, videoSrc, videoFrameCount = 24, alt, className }: SpinViewerProps) {
  const [frames, setFrames] = useState<string[]>(framesProp ?? []);
  ...
  useEffect(() => {
    ...
    extractFramesFromVideo(videoSrc, { frameCount: videoFrameCount })
      .then((extracted) => {
        if (!cancelled) setFrames(extracted.map((f) => f.dataUrl));
      })
      .catch(() => {
        // Leave whatever frames were already set (prop or empty)
      });
  ...
  const stepFrame = useCallback(
    (delta: number) => setCurrentFrame((f) => mod(f + delta, frameCount)),
    [frameCount],
  );
  ...
  return (
    <div
      ...
      className={cn(
        "relative aspect-square select-none overflow-hidden bg-[var(--color-surface-raised)] outline-none touch-none",
    ...
    >
      {!allLoaded && (
        <div className="absolute inset-0 flex flex-col items-center justify-center gap-3">
          <div className="h-px w-24 bg-[var(--color-border)] overflow-hidden">
            <div
      ...
      {allLoaded && showHint && (
        <div className="absolute inset-x-0 bottom-4 flex justify-center pointer-events-none transition-opacity duration-500">
          <span className="font-mono text-[10px] uppercase tracking-widest text-[var(--color-text-secondary)] bg-[var(--color-surface)]/90 px-3 py-1.5 border border-[var(--color-border)]">
            Drag to rotate
          </span>
        </div>
      )}
      ...
      {allLoaded && (
        <>
          <span className="absolute bottom-4 right-4 font-mono text-[10px] tracking-wide text-[var(--color-text-secondary)] bg-[var(--color-surface)]/90 px-2 py-1">
          ...
          <button
            ...
            className="absolute top-4 right-4 w-9 h-9 flex items-center justify-center bg-[var(--color-surface)]/90 border border-[var(--color-border)] text-[var(--color-ink)] hover:border-[var(--color-border-strong)] transition-colors"
          >
            {isZoomed ? (
